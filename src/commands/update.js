...
module.exports.execute = async (client, message, args, send) => {
	...
	require('request')({ url: `https://api.henrikdev.xyz/valorant/v1/website/en-us`, headers: { "Authorization": process.env.HD_KEY } }, async (err, res, body) => {
		...
		let row2 = new Discord.ActionRowBuilder()
			.addComponents([
				new Discord.ButtonBuilder()
					.setLabel("View on valorant website")
					.setStyle("Link")
		...
		const updateInfo = () => {
			...
			if (data[page].external_link) {
				row2.components[1].setDisabled(false)
				row2.components[1].setURL(data[page].external_link)
			}
			else row2.components[1].setDisabled(true)
		}

		updateInfo()

		const initial = await send(message, { embeds: [emb], components: [row, row2] })

		const filter = i => i.user.id === message.author.id
		const coll = initial.createMessageComponentCollector({ filter, time: 80000, errors: ['time'] })
