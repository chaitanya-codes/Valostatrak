...
module.exports.execute = async (client, message, args, send) => {
	...
	if (subcommand === 'find') {
		...
		if (!args.join(" ").includes("#")) return message.reply("ID not found. Usage: `v!account <name#tagg>`\nExample: `v!account 100T Asuna#1111`")
		...
		args.shift()
		const linked = client.linked.get(message.author.id)
		...
		col.on("collect", i => {
			if (i.customId === 'private') {
				if (!linked.private) client.linked.set(message.author.id, true, 'private')
					else client.linked.set(message.author.id, false, 'private')
						i.deferReply()
					if (linked.private) row.components[0].setLabel("Statistics are private").setStyle(EnumResolvers.resolveButtonStyle("DANGER"))
						else row.components[0].setLabel("Statistics are public").setStyle(EnumResolvers.resolveButtonStyle("PRIMARY"))
							send(m, {edit: true, components: [row]})
					}
